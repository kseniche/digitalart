 ...
 * например: Анна-Мария, Жан-Пьер, Салтыков-Щедрин.
 */
class PersonNameLetters implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (! preg_match('/^[\p{L}]+(?:[ \x{27}\x{2019}\-][\p{L}]+)*$/u', $trimmed)) {
            $fail('Допускаются только буквы; части имени или фамилии разделяйте одним пробелом, дефисом или апострофом (например, Анна-Мария, Жан-Пьер, Салтыков-Щедрин, О\'Коннор).');
        }
    }
}
